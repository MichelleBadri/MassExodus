...
def doit(n,qmod,qcon,beta,T,outfile,p):
    G = nx.Graph()
    ...
    # Fill a ndarray vector of size n*40 with random ints between 0 and 1.
    randArray = np.random.rand(1,n*500)
    # Convert 1D array to list for pop 
    randArray = randArray[0].tolist()

    # Header. - Printed optionally
    if p == True:   
    	# print "#Iter\tNodes\tInPlay\tIso\tEdges\tNmEval\tComps\tMIS\tDensity\tCC\tTris\tFracDeg1\tFracDeg0\tNGcc\tMGcc"
        print "#Iter\tNodes\tInPlay\tIso\tEdges\tNmEval\tComps\tMIS\tDensity\tCC"
    for iter in xrange(1,T+1):
        ...
        if len(IsolatedPool) != 0:
            ...
            for neighbor in G.neighbors(u):
                assert neighbor != u
                if randArray.pop() < qmod: # modify the edge.  	# Random number from numpy array						## REPLACE WITH RANDARRAY
                    if randArray.pop() < 0.5: # delete u->neighbor.  # Random number from numpy array
                        Delete.add(neighbor)
                        G.add_edge(v,neighbor)
                    #else: # delete v->neighbor -- aready done.
            ...
            if randArray.pop() < qcon: 	# Random number from numpy array
                assert u != v
        ...
        (nmeval,comps,mis,density,cc) = F.compute_features(G)
        m = G.size()
      
        # 8. Output results - Printed optionally
        if p == True:
            ...
            print "%i\t%i\t%i\t%i\t%i\t%.5f\t%i\t%.5f\t%.5f\t%.5f" %(iter,G.order(),len(InPlayPool),len(IsolatedPool),m,nmeval,comps,mis,density,cc)
...
def main():
    ...
    parser.add_argument('-p', '--printres',type=bool, help='print the results', default=False) ### Add flag for printing results
    ...
    beta = args.beta
    T    = args.niter
    p    = args.printres # Print computed distances for each iteration optionally

    if args.outfile is None:
        # outfile = "massexodus-%i-%.1f-%.1f-%.2f.graph" %(n,qmod,qcon,beta) 
        outfile = "massexodus-%i-%.2f.graph" %(n,beta)
    else:
...
#    beta = float(sys.argv[4])
    ...
    doit(n,qmod,qcon,beta,T,outfile,p)
